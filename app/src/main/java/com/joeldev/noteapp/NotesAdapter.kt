...
class NotesAdapter(
    ...
    private val listener: OnNoteItemRemovedListener,
    private val onNoteSelected: (Int) -> Unit
) :
    RecyclerView.Adapter<NotesViewHolder>() {

    interface OnNoteItemRemovedListener {
        fun onNoteItemRemoved(position: Int)
    }
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        holder.render(notes[position].title)
        ...
        holder.itemView.findViewById<ImageButton>(R.id.ibtnMenu).setOnClickListener {
            ...
            popupMenu.setOnMenuItemClickListener { menuItem ->
                when (menuItem.itemId) {
                    ...
                    2 -> {
                        listener.onNoteItemRemoved(position)
                        notifyDataSetChanged()
